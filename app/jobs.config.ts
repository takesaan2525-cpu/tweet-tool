...
export const JOBS: JobDef[] = [
  ...
  {
    /* ★2026-08-06 追加。エス魂だけは並び順を機械的に揃えられる。
       ・駅ちか/MAP/リフナビ/エステラブ/ランキングは既にHPと一致（触る必要なし）
       ・じゃぱんは媒体側に25名を並べ替えるUIが無く、直す手段がない
       実行前に今の並びを estama_sort_backup_*.json に残すので元に戻せる。 */
    id: 'estama_sort',
    name: 'エス魂の並びをHPに揃える',
    desc: 'エス魂のセラピスト表示順を、HPと同じ並びにします（実行前に今の並びを控えるので元に戻せます）',
    kind: 'cast',
    script: 'estama_sort.js',
    cooldownSec: 300,
    confirm: true,
  },
  {
